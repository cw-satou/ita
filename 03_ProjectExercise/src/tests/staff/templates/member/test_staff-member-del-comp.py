...
@app.route("/staff", endpoint="staff.member_list")
...
# スタッフ用会員退会完了画面のテンプレートをテストする関数
def test_staff_member_del_comp_01(client):
    """No.01 テストケース名：テンプレート「会員退会完了画面」表示"""

    ### Arrange（準備） ###
    # テスト対象のテンプレートを定義
    template_file = "member/staff-member-del-comp.html"
