...
# スタッフ用会員一覧画面のテンプレートをテストする関数
def test_staff_member_list_01(client):
    """No.01 テストケース名：テンプレート「会員情報一覧画面」表示"""

    ### Arrange（準備） ###
    # テスト対象のテンプレートを定義
    template_file = "member/staff-member-list.html"
    ...
    member = MemberMaster()
    member.user_id = 2
    member.member_name = "会員名２"
    member.address = "住所２"
    member.email = "kaiin2@sample.com"
    member.phone_number = "09022222222"
    members.append(member)

    # 3人目の会員データを追加
    member = MemberMaster()
    member.user_id = 3
    member.member_name = "会員名３"
    member.address = "住所３"
    member.email = "kaiin3@sample.com"
    member.phone_number = "09033333333"
    members.append(member)
    ...
    with app.test_request_context():
        ...
        rendered_html = render_template(template_file, members=members)
