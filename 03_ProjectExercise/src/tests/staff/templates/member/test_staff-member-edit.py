...
# スタッフ用会員変更画面のテンプレートをテストする関数
def test_staff_member_edit_01(client):
    """No.01 テストケース名：テンプレート「会員変更変更画面」表示"""


    ### Arrange（準備） ###
    # テスト対象のテンプレートを定義
    template_file = "member/staff-member-edit.html"
    ...
    # 入力エラーのテスト用メッセージを設定
    err_member_name = "会員名入力エラー"
    err_address = "住所入力エラー"
    err_email = "メールアドレス入力エラー"
    err_phone_number = "電話番号入力エラー"
