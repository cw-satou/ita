...
# スタッフ追加画面テンプレートをテストする関数
def test_staff_add_01(client):
    """No.01 テストケース名：テンプレート「スタッフ追加画面」表示"""

    ### Arrange（準備） ###
    # テスト対象の追加画面テンプレートを指定
    template_file = "staff/staff-add.html"

    # 新規スタッフのテストデータを作成（入力値とエラーメッセージの両方を設定）
    staff = StaffMaster()
    staff.staff_id = 1
    ...
    with client.application.test_request_context():
        ...
        # テンプレートに新規スタッフデータとエラーメッセージを渡してHTML生成
