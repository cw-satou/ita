...
# スタッフ追加完了画面のテンプレートをテストする関数
def test_staff_add_comp_01(client):
    """No.01 テストケース名：テンプレート「スタッフ追加完了画面」表示"""

    ### Arrange（準備） ###
    # テスト対象の追加完了画面テンプレートを指定
    template_file = "staff/staff-add-comp.html"

    # 新規追加されたスタッフのテストデータを作成
    staff = StaffMaster()
    staff.staff_id = 1
